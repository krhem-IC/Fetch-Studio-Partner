...
def render_offer_detail_page():
    ...
    if uploaded_file is not None:
        try:
            ...
            with col2:
                bg = st.selectbox("Expected Background Color", [f"{p['name']} {p['hex']}" for p in APPROVED_SWATCHES], key="upload_bg_detail")
                lifestyle = st.text_input("Lifestyle keywords")
            
            bg_hex = bg.split()[-1]
            
            # Process and validate
            img_processed = enforce_specs(img, "offer_detail")
            report = validate_image(img_processed, "offer_detail", bg_hex)
            
            # Generate prompt for reference
            if brand and products:
                product_list = [p.strip() for p in products.split(",") if p.strip()]
                prompt = build_prompt("offer_detail", brand, product_list, bg_hex, lifestyle)
            ...
            if report["pass"]:
                ...
                filename = suggest_filename("offer_detail", bg_hex, uploaded_file.name)
